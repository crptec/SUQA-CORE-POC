...
Below are some notes on how to build Suqa Core for Windows.

The options known to work for building Suqa Core on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Bionic 18.04 is required
and is the platform used to build the Suqa Core Windows release binaries.
...
Note that for WSL the Suqa Core source path MUST be somewhere in the default mount file system, for
example /usr/src/suqa, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\suqa`, for example:

    make install DESTDIR=/mnt/c/workspace/suqa
...
It's not possible to build the Suqa Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Suqa Core source code).
