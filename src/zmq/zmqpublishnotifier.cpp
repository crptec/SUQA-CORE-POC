...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    if (i==mapPublishNotifiers.end())
    {
        ...
        LogPrintf("zmq: Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
    ...
    {
        LogPrintf("zmq: Reusing socket for address %s\n", address);
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrintf("zmq: Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrintf("zmq: Publish hashblock %s\n", hash.GetHex());
    char data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrintf("zmq: Publish hashtx %s\n", hash.GetHex());
    char data[32];
    for (unsigned int i = 0; i < 32; i++)
        data[31 - i] = hash.begin()[i];
...
{
    LogPrintf("zmq: Publish rawblock %s\n", pindex->GetBlockHash().GetHex());
